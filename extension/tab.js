var phrases = {
  ...
  bank:
    "A shot that first hits the backboard before going through the rim. The square on the backboard helps players aim bank shots from angles other than straight ahead.",
  "big man":
  ...
  cut:
    "An action made by offensive players in which they quickly accelerate in a different direction, usually towards the basket in the hope of catching the defense off guard and scoring an easy basket.",
  "defensive rebound":
  ...
  dunk:
    "An offensive action in which a shot is made by delivering the ball straight into the rim without releasing a shot.",
  elbow:
  ...
  flop:
    "An overreaction or exaggerated movement done with the intention of drawing a foul on the other team that may or may not be deserved.",
  foul:
    "Illegal contact made with the opposition on either side of the ball. This can include hitting a shooter on the arm, running over a defender, or grabbing a dribbling opponent. When the offensive player is in the act of shooting, foul shots are rewarded.",
  "four point play":
  ...
  key:
    "Another name for the paint or the lane, usually painted another color than the surrounding area within the perimeter.",
  lane:
    "Another name for the “paint” or “key.” The area runs from the baseline under the basket to the free throw line and from block to block.",
  "lay up":
  ...
  wing:
    "1. An area located on either side of the court, beyond the three-point line, lined up with the free-throw line. 2. A term used to describe a player that plays in the area defined as the wing. These are usually shooting guards and small forwards.",
  "zone defense":
...
// Convert "%20" to spaces and remove moreInfo
function convertToSpaces(phrase) {
  phrase = phrase.replaceAll(/%20/g, " ");
  return phrase.replace(moreInfo, "");
}

var uppercasePhrase = convertToSpaces(location.hash.slice(1));
var phrase = makeLowercase(uppercasePhrase);

var definition = phrase.repeat(15);
if (phrase in phrases) {
  definition = phrases[phrase];
}

var lingoContainer = document.getElementsByClassName("lingoContainer")[0];
var lingo = document.createElement("div");
lingo.textContent = uppercasePhrase;
lingo.style.fontSize = "20px";
lingoContainer.appendChild(lingo);

var defContainer = document.getElementsByClassName("defContainer")[0];
var def = document.createElement("div");
def.textContent = definition;
def.style.fontSize = "20px";
defContainer.appendChild(def);

// // Remove hash from url. Refreshing is messed up though
// history.pushState(
//   "",
//   document.title,
